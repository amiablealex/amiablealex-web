...
- **Algorithm to select the daily ingredient** and how it evolved from completely random. i.e truly random wasnt a good user experience. i'd explain how the actual algorithm works with the weighting, never-selected weights, same-property penalisers etc.
...
- **complexity of the entire fuzzy matching / spell check** (i.e all 8 steps of checking for aliases, etc...) and turning bugs into features (the easter-egg philosophy). i.e Custard Conundrum — fuzzy matching would "correct" custard to mustard, so instead of fixing it became an achievement.
